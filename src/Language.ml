...
module MyUtils =
  ...
  let bool_to_int = function true -> 1 | _ -> 0
  let int_to_bool b = b <> 0
  let toFunc op =
    let (|>) f g   = fun x y -> f (g x y) in
      ...
      | "==" -> bool_to_int |> (= )
      | "!=" -> bool_to_int |> (<>)
      | "&&" -> fun x y -> bool_to_int (int_to_bool x && int_to_bool y)
      | "!!" -> fun x y -> bool_to_int (int_to_bool x || int_to_bool y)
    ...
    *)
    ...
    let rec eval env ((st, i, o, r) as conf) k stmt = match stmt with
      ...
      | If (e, thenStmt, elseStmt) -> let ((st, i, o, Some r) as conf') = Expr.eval env conf e in
                                        if MyUtils.int_to_bool (Value.to_int r) then eval env conf' k thenStmt
      ...
      | ForEach (x, e1, e2, b)            -> if Expr.Binop ("<=", Expr.eval st e1, Expr.eval st e2) then
